...
const layout = [
  ...
  { i: "mapEditSecTim", x: 4, y: 5, w: 4, h: 10 },
...
export const StrategyPlanner = () => {
  ...
  const [secTimConfigEn, setSecTimConfigEn] = useState(false); //Enables the sector timer view
  ...
  function mqttRxCallback(message) {
    ...
    if (message.topic == lastDispTopic) {
      ...
      setCarIndex(
        bgMapRef.current.playPosition(
          { lat: parseFloat(record[0]), lng: parseFloat(record[1]) },
          stratMap
        )
      );
      //carIndex = bgMapRef.current.playPosition(
      //{ lat: parseFloat(record[0]), lng: parseFloat(record[1]) },
      //stratMap
      //);
    }
  ...
  useEffect(() => {
    if (!didMount.current) {
      //Prevent running this when it's the first rendering
  ...
  const loadData = (file: File) => {
    ...
    fileData.onloadend = (e) => {
      ...
      let isMapComplete =
        strategyIndex > 0 && sectorIndex > 0 && altitudeIndex > 0; //Check if all data is present
      ...
      for (let i = 1; i < fileByLines.length - 1; i++) {
        ...
        if (isMapComplete) {
          stratMap.push({
            id: parseInt(record[idIndex]),
            pos: {
              lat: parseFloat(record[latIndex]),
              lng: parseFloat(record[lngIndex]),
            },
            altitude: parseFloat(record[altitudeIndex]),
            dataToDisplay: 0,
            strategy: parseInt(record[strategyIndex]),
            sector: parseInt(record[sectorIndex]),
            note: "",
            speed: 0,
          });
        } else {
  ...
  const sendData = () => {
    ...
    if (carSelect == "1") {
      fetch("http://h2polito.duckdns.org/JunoFile", {
        // Server IP address
        ...
        //When response received notify phone file is online
        console.log("DONE");
      ...
      fetch("http://h2polito.duckdns.org/IdraFile", {
        // Server IP address
    ...
    <div className="mb-auto mx-auto">
      ...
      >
        ...
        {mapConfigEn && (
          <div
            key="mapEditTable"
            data-grid={{ x: 16, y: 2, w: 6, h: 15, minW: 5 }}
          >
            <MapConfigurator
              putMarker={bgMapRef.current.putMarker}
              setBlkConfigEn={setBlkConfigEn}
              setMapConfigEn={setMapConfigEn}
              mapData={stratMap}
              setMapData={setStratmap}
              setSecTimConfigEn={setSecTimConfigEn}
            ></MapConfigurator>
          </div>
        )}
        ...
        {blkConfigEn && (
          <div key="mapEditBulk" data-grid={{ x: 0, y: 5, w: 4, h: 10 }}>
            <BulkEditor
              ...
              updateMap={bgMapRef.current.updatePath(stratMap)}
              setBlkConfigEn={setBlkConfigEn}
              mapData={stratMap}
              setMapData={setStratmap}
            ></BulkEditor>
        ...
        {secTimConfigEn && (
          <div key="mapEditSecTim" data-grid={{ x: 4, y: 5, w: 4, h: 10 }}>
            <SectorTimer
              pointIndex={carIndex}
              setSecTimConfigEn={setSecTimConfigEn}
              mapData={stratMap}
        ...
        {settingsEn && (
          <div key="settings" data-grid={{ x: 1, y: 2, w: 5, h: 9 }}>
            <Settings
              setSettings={setSettingsEn}
              carSelect={carSelect}
              dispDataEn={dispDataEn}
              setDispDataEn={setDispDataEn}
