...
export const PostRun = (props) => {
  ...
  useEffect(() => {
    plotRefWheelIdra.current = new uPlot(
      optsWheelIdra,
      dataWheelIdra,
      plotRefWheelIdra.current
    );
  }, [dataWheelIdra]);

  useEffect(() => {
    plotRefVoltage.current = new uPlot(
      optsVoltage,
      dataVoltage,
      plotRefVoltage.current
    );
  }, [dataVoltage]);

  useEffect(() => {
    plotRefCurrents.current = new uPlot(
      optsCurrents,
      dataCurrents,
      plotRefCurrents.current
    );
  }, [dataCurrents]);

  useEffect(() => {
    plotRefAltimetry.current = new uPlot(
      optsAltimetry,
      dataAltimetry,
      plotRefAltimetry.current
    );
  }, [dataAltimetry]);

  useEffect(() => {
    plotRefSpeed.current = new uPlot(
      optsSpeed,
      dataSpeed,
      plotRefSpeed.current
    );

    if (dataSpeed.length != 0) setIsFilePicked(true);
  }, [dataSpeed]);
  ...
  useEffect(() => {
    if (trackPoints != undefined) {
      ...
      const newMap = L.map("mapId").setView(
  ...
  function playCar(tp) {
    let i = 0;
    let marker = L.marker(
      [tp[i % (tp.length - 1)][0], tp[i % (tp.length - 1)][1]],
      { icon: carIcon }
    );
  ...
  const fileInsertion = (event) => {
    ...
    reader.onload = async (e) => {
      ...
      if (event.target.files[0].name.slice(0, 4) === "IDRA") {
        ...
        setDataWheelIdra([
          result.map((x, ind) => x["TIME"] / 1000 / 60),
      ...
      setTrackPoints(
        result.filter(filterNanPoints).map((p) => {
