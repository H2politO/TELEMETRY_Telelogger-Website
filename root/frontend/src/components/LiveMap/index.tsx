...
export class LiveMap extends Component<any> {
    ...
    changeHandler = (event) => {
        ...
        reader.onload = async (e) => {
            console.log(JSON.parse(reader.result.toString()).features[0].geometry);
            
            let geoJsonMap= L.geoJSON(JSON.parse(reader.result.toString()).features[0].geometry).addTo(this.map).setStyle({ color: 'red', weight: 5 });
            this.map.setView([
                this.px, this.py
            ], 10);
    ...
    render() {
        return (
            <div>
                ...
                <div className="parentMap" id="circuitMap">
                </div>
            </div>
