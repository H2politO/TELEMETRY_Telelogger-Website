...
export const BulkEditor = ({
  ...
  const updateIndexes = (event, isEndIndex) => {
    ...
    if (isEndIndex) {
      if (tmp > startIndex) setEndIndex(tmp);
    } else {
      if (tmp < endIndex) setStartIndex(tmp);
    }
  };

  const setStrat = (event) => {
    let tmp = parseInt(event.target.value);
    if (tmp < -1 || tmp > 20) return; //Avoid problems
    setStrategy(tmp);
  };
  ...
  const updateData = () => {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      if (strategy != -1) tmpData[i].strategy = strategy;
      if (sector != -1) tmpData[i].sector = sector;
    }

    setMapData(tmpData);
    setStartIndex(endIndex);
    if (endIndex + 1 < mapData.length) setEndIndex(endIndex + 1);
  };
      ...
      <div id="main" style={Styles.baseStyle}>
        ...
        <div id="rightColumn" style={Styles.rightColumnStyle}>
          ...
          <OutlinedInput
            ...
            value={startIndex}
            onChange={(event) => {
              updateIndexes(event, false);
            }}
          />
        </div>
      </div>
      <Button style={Styles.buttonStyle} onClick={updateData}>
        {" "}
        EDIT
      </Button>
    </div>
